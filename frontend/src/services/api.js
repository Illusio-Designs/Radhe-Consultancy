...
api.interceptors.request.use(
  ...
  (error) => {
    console.error('\n=== API Request Error Debug Log ===');
    console.error('Request Error:', error);
    console.error('Environment:', import.meta.env.MODE);
    console.log('=== End API Request Error Debug Log ===\n');
...
api.interceptors.response.use(
  (response) => {
    ...
    console.log('- Environment:', import.meta.env.MODE);
    console.log('=== End API Response Debug Log ===\n');
    return response;
  },
  (error) => {
    ...
    if (error.response) {
      ...
      if (error.response.status === 401) {
        console.log('Unauthorized error (401) detected');
