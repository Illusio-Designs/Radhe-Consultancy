...
import { employeeCompensationAPI, insuranceCompanyAPI } from "../../../services/api";
...
const PolicyForm = ({ policy, onClose, onPolicyUpdated }) => {
  ...
  const handleCompanyChange = (e) => {
    ...
    const selectedCompany = companies.find(company => company.company_id === parseInt(selectedCompanyId));
    if (selectedCompany) {
      console.log('GST Number:', selectedCompany.gst_number, 'PAN Number:', selectedCompany.pan_number);
    }
    setFormData(prev => ({
      ...prev,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const errorMessage = err.response?.data?.error || err.response?.data?.message || "Failed to save policy";
      setError(errorMessage);
      toast.error(errorMessage);
    } finally {
      setLoading(false);
