...
const sequelize = new Sequelize({
  database: 'radhe_consultancy_crm',
  username: 'root',
  password: '',
  host: 'localhost',
  port: 3306,
...
async function analyzeForeignKeys() {
  try {
    ...
    const logs = await sequelize.query(
      'SELECT * FROM user_role_work_logs ORDER BY id ASC',
    ...
    for (const log of logs) {
      try {
        ...
        const users = await sequelize.query(
          ...
          { 
            replacements: [log.target_user_id],
            type: sequelize.QueryTypes.SELECT 
          }
        );
        
        if (users.length > 0) {
          console.log(`  ✓ Valid user: ${users[0].email}`);
          validLogs++;
          continue;
        }
        ...
        const companies = await sequelize.query(
          'SELECT company_id, company_name, user_id FROM Companies WHERE company_id = ?',
          { 
            replacements: [log.target_user_id],
            type: sequelize.QueryTypes.SELECT 
          }
        ...
        if (companies.length > 0) {
          ...
          const consumers = await sequelize.query(
            'SELECT consumer_id, consumer_name, user_id FROM Consumers WHERE consumer_id = ?',
            { 
              replacements: [log.target_user_id],
              type: sequelize.QueryTypes.SELECT 
            }
